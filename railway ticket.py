...
class train:
    def __init__(self,name='',num=0, arr_time='',dep_time='',src='',des='',day_of_travel='',seat_available_in_1AC=0,seat_available_in_2AC=0,seat_avalable_in_SL=0,fare_1ac=0,fare_2ac=0,fare_sl=0):
        self.name=name
        self.num=num
        ...
        self.seats={'1AC': seat_available_in_1AC, '2AC':seat_available_in_2AC,'SL':seat_avalable_in_SL}
...
class ticket:
    def __init__(self,train,user,ticket_num,coach):
        self.pnr=str(train.num)+str(user.uid)+str(random.randint)+str(train.name)+str(coach.coach)+(user.name)
        self.train_num =train.num
        self.coach =coach
        self.uid =user.uid
        self.train_num=train.name
...
class acceptors:
    ''' Class containing functions for accepting
     and validating values properly'''
    def accept_uid():
        uid =0
        try:
...
def book_ticket():
    ...
    ticket_num =acceptors.accept_train_number()
...
def check_pnr(pnr = ''):
    if pnr == '':
        ...
        print('Train number:= ',ticket_dict[pnr].train_num,'source')
        print('No. of Tickets Booked := ',ticket_dict[pnr].ticket_book)
        print()
        menu()
    else:
        print()
...
t2 =train('howrah',12565,'02:34','23:12','hwr','kol','mon',4,12,3434,435,234)
...
def menu():
    ...
    print('3.Check PNR')
    print('4.Check seat availablty')
    print('5.Create new account')
    print("6.Check previous booking")
    print("7.Login")
    print('8.Exit')
    func = {1:book_ticket, 2: cancel_ticket,3:check_pnr,4:check_seat_availabilty,5:create_new_acc,6:check_prev_booking,7:login,8:exit}
    options = acceptors.accept_menu_option()
    func[options]()
